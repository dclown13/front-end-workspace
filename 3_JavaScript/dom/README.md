...
- 두번째 : 창 이름, 창 이름이 같은게 이미 열려있을 경우 새로 열리지 않고   
          이미 열려있는 곳에서 새로고침
...
- 태그의 class 속성 값을 이요해서 객체를 배열에 담아 가져온다.
- 동일한 class 속성 값을 갖는 태그가 여러개 존재할 수 있기 때문에 배열로 리턴

```js
document.gatElementsByClassName("클래스");
```
...
```js
document.gatElementsByTagName("태그");
```
